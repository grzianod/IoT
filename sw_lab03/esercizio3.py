...
if __name__ == "__main__":
	...
	publisher  = Publisher("MBP-di-Graziano", "test.mosquitto.org", 1883)
	...
	if "arduino" not in devices.keys():
	...
	while(True):
		...
		publisher.onPublish(subscriptions.get('device').get('MQTT').get('topic')+"/"+devices.get('arduino').get('deviceID')+"/led", json.dumps(payload))
		time.sleep(15)
